...
**Status:** v0, declared 2026-09-15; reference implementation `0.3.1`. The conformance suite passes, the §9 questions are decided, and the naive-output contract has been verified on GitHub.
...
The suite is distributed as `@markset-lang/conformance-suite`, which carries those files and that schema and nothing else — no parser, no renderer, no dependency on any implementation — so that checking an implementation does not require obtaining another one. The JSON is the artifact; an implementation in any language reads it directly.
...
- **`chart`** — no longer deferred, and never became a construct. Settled in §11 on the same grounds as §10: a chart is a `figure` holding a table, which independent renderers already agree on, so what was missing was a statement of what a renderer may do with one rather than a ninth name. The three worries recorded here — a data format decision, a rendering library, a print story — are answered there: the table is the format, and the rest follows from it.
...
A chart is **not a construct**, for the same reason a diagram is not (§10). A chart is a `figure` whose content is a table and whose specifier names a chart type:

````markdown
:::figure[Requests per second by region, by quarter]{#fig-rps chart=line}
...
Everything there except `chart=line` already worked. `figure` has always taken a table as its content (§4.8), a GFM table is a table, and independent renderers already produce the same `<figure>` and `<table>` from it. That structural agreement is the only thing a construct could have bought. What renderers did not agree on is whether to *draw* it, and that is what this section settles.

**The source is the table, and that is the part of this section that is about Markset rather than about charts.** Section 3 requires a document to read where the layout cannot follow, and §10 had to settle for a diagram fence degrading to its own source, because a diagram's content has no canonical text form. A chart's does: it is the data. So the naive output of a chart is the table it was drawn from — not a gesture at the content but the content itself, in every output target, with no engine installed. This is why a chart takes a table rather than a fence naming a data language. A `vega-lite` or `csv` fence would either duplicate the table a quantitative document was going to contain anyway, or replace it with something no reader reads, and both are worse than the table.

**How a table becomes a series.** Renderers that draw must agree on this or the same document is two different pictures:
...
- A cell that is not a number is a missing point, not a zero. A series may have gaps.

Anything beyond that — scales, gridlines, tick density, where a legend sits, which color a series takes — is the theme's, not the document's, and not this specification's. That line is invariant 1 applied to charts, and it is where charts will erode it if it is not written down: a document names what its data means and never how the picture looks.

**Renderer obligations.** For a `figure` carrying a `chart` attribute:

1. A renderer that does not draw charts MUST render the figure and its table as it would any other. This is the default and it is always correct.
2. A renderer that draws charts MAY add a drawn form to the HTML output.
3. Drawing MUST NOT change the AST. A chart is a `figure` holding a `table` before and after, so the `ast` aspect of a chart case is the same whether or not a renderer can draw it.
4. The downgrade output of a chart is the table, unchanged, always. Drawing is an HTML-only affordance.
5. If drawing fails, the renderer MUST fall back to the table and SHOULD report the failure out of band. **A failed chart never removes content.**
6. A drawn form MUST be inert: no script, and no fetch at display time. §10 gets this from the shape of its output — an SVG loaded through `<img>` cannot execute — and a renderer that draws inline does not, so it MUST hold the property in how it builds the picture instead.
7. A renderer MUST NOT draw a chart for a figure with no caption. The caption is what says what the picture shows, and a drawn chart without one claims more than it can support.
8. **A drawn chart MUST NOT make its table unreachable.** This is the obligation charts have and diagrams do not, and it is the reason the list is eight and not seven.

**Why obligation 8 exists.** An undrawn diagram is a code block: mediocre for a reader who cannot see a picture, but present, which is what §10's obligation 7 protects. A chart is different in a way that cuts the other direction. Its picture is a *lossy view of data that is sitting right there* — a reader can want the exact figure behind a point, and in a document where the numbers are the argument, that reader is not an edge case. Replacing the table with an image would therefore discard something every reader could have used, not only one who cannot see it. So the drawn form is an addition to the table, never a substitution for it. How the two are arranged — side by side, the table folded into a `<details>`, the picture first — is presentation, and belongs to the theme.

**Which chart types.** The set is closed, which is the opposite of §10's open language set, and the asymmetry has a reason. An unknown diagram language is safe because obligation 1 turns it back into a code block that says exactly what the author wrote. An unknown chart type is not: the author has asked for a specific reading of their data, and a renderer that guesses draws a different argument from the same table. So a type outside the set is `DIRECTIVE_INVALID_ATTRIBUTE` and the figure stays a plain table.
...
`bar` and `column` are the same encoding — length against a baseline — on opposite axes, and it is worth saying why that is two type names rather than one type and an orientation. An orientation would be presentational, and invariant 1 keeps presentation out of the document. Which axis carries the categories is not: this section already defines the category axis as a property of the data, the choice follows from the data rather than from taste (long category names and ranked lists read down; ordered periods read across), and a theme could not make the choice even if it were given it, because a drawn chart reaches the page as an image and the theme cannot see inside it. So it is the document's, on the same footing as the `columns` ratio.

That is still smaller than the set will end up, deliberately: adding a type is an additive change and removing one is a breaking change, so the set grows on evidence rather than on anticipation. `docs/future-requirements.md` entry 17 carries the evidence behind each of these and the reasoning for what is not here yet — including the proportion charts that were considered and declined, and the ones that need a different table mapping than the one above.

**Validation.** A `chart` attribute on a figure whose content is not a table is the error `CHART_CONTENT`, and the figure is rendered as an ordinary figure. A chart is drawn from data and the other two figure contents carry none: an image is already a picture, and a code fence is §10's business.

**HTML.** The chart type reaches HTML as `data-chart` on the `<figure>`, whether or not the renderer can draw: `<figure class="ms-figure" id="fig-rps" data-chart="line">`. A renderer with no chart engine stops there, and the output is the table with its caption (§4.8), which is obligation 1.

A renderer that does draw puts the picture before the table, inside the same figure. The specification does not say what the picture is made of; what follows is what the reference implementation does and why, because the choice is not obvious and §10 made the opposite one.

**A drawn chart is inline `<svg>`, where a drawn diagram is an `<img>` holding a data URI.** §10 refuses inline markup because a diagram engine there may be an arbitrary command named by the operator, and raw markup from one is a channel documents are denied (§4); wrapping it in an `<img>` makes obligation 6 true by the shape of the output rather than by trusting the engine. A built-in chart engine is not an arbitrary command: it is part of the renderer, it builds nodes rather than emitting text to be parsed, and so there is no markup channel to open. That is why the same reasoning lands differently here, and why obligation 6 is written to allow both.

What inline buys is the thing a data URI cannot give at any price: **a chart the theme can color.** The reference engine emits no color at all. Its output carries geometry and a series number, `data-series="1"`, and the stylesheet supplies every color — so a theme recolors charts by setting `--ms-chart-1`, exactly as it restyles anything else (§6). That is invariant 1 applied to a picture: the document says which series a mark belongs to and the stylesheet decides what that looks like. A drawn diagram cannot work this way, which is a real cost of §10's choice rather than an inconsistency in this one.

The picture is `aria-hidden`. The table beside it carries every value, and the caption names the figure, so announcing the picture as well would say the same thing twice — the same reasoning as the step markers in §4.6. Obligation 8 is what makes that safe: the data is never only in the picture.

**In the conformance suite, chart cases pin the undrawn form.** A drawn chart is an implementation's own SVG, which a second implementation cannot reproduce byte for byte, so it could never be a shared expectation. Obligation 1 makes the table the form every conformant renderer agrees on, and that is what the `html` aspect of a chart case records.
