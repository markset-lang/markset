...
v0 defines one type, `line`, which draws each series as a line over the category axis. That is smaller than the set will end up, deliberately: adding a type is an additive change and removing one is a breaking change, so the set starts at what a document has actually asked for and grows on evidence. `docs/future-requirements.md` entry 17 carries the evidence for `line` and the reasoning for what is not here yet.
