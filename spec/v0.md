...
Throughout this section a **drawer** means whatever turns a diagram fence into a picture: a function inside the renderer, or a program the renderer runs. The specification says nothing about how one works or where it comes from, and obligations 1 and 5 are written so that having no drawer, or having one that fails, is always safe.
...
**Which languages.** The set is open and deliberately unspecified. A closed list would make a renderer that supports one more language non-conformant for no gain, and obligation 1 already makes an unknown language safe. `markset check` does not validate diagram source and reports no diagnostic for an unknown one: the language is the drawer's business, not the parser's.

**HTML.** A drawn diagram is `<img class="ms-diagram" data-diagram="L" src="…" alt="…">` in place of the `<pre>`, with `alt` taken from the caption. The reference implementation draws to SVG and embeds it as a `data:` URI rather than inlining the markup, for the reason in §4: raw HTML in the source is never passed through, and a drawer should not be handed a channel that documents are denied. An SVG loaded through `<img>` cannot execute script, so obligation 6 holds by construction rather than by inspection. A drawer that wants to follow the reader's color scheme carries its own `prefers-color-scheme` block, which resolves against the embedding page's `color-scheme` (§6).
