...
5. §5 says reserved classes apply to "any block via an attribute specifier", but v0 defines no syntax for attaching a specifier to a paragraph or heading. Candidates are djot's attribute line before the block and Pandoc's trailing `{...}` on headings. Until decided, `.lead` and `.eyebrow` are reachable only through spans and directives.
