...
6. A drawn form MUST be inert: no script, and no fetch at display time.
...
**HTML.** The chart type reaches HTML as `data-chart` on the `<figure>`, whether or not the renderer can draw: `<figure class="ms-figure" id="fig-rps" data-chart="line">`. A renderer with no chart engine stops there, and the output is the table with its caption (§4.8) — which is obligation 1, and is what the reference implementation does today.
