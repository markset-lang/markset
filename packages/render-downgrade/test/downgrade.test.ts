...
test("downgrade output contains no Markset syntax", () => {
  const source = [
    "---", "markset: 0", "---",
    "# Doc",
    "::::columns{ratio=\"2:1\"}", "Main [Beta]{.badge}.", "", "::col", "", ":::card[Side]{tone=info}", "> [!NOTE] Hi", "> body", ":::", "::::",
    ":::tabs", "### A", "a", "### B", "b", ":::",
    ":::figure[Cap]{width=50%}", "![](x.svg)", ":::",
  ].join("\n") + "\n";
  const { markdown } = downgrade(source);
...
test("card titles become headings one level below the current section", () => {
  const { ast } = parseDocument("# H1\n\n:::card[A]\nx\n:::\n\n### H3\n\n::::card[B]\n:::card[C]\ny\n:::\n::::\n\n:::card[D]\n:::\n");
  const depths = downgradeTree(ast).children.filter((n) => n.type === "heading").map((n) => n.depth);
