...
import type {
  Callout, CalloutKind, Card, Column, Columns, Directive, Figure, Gap, Grid, Metrics, SeparatorNode, Steps, Tab, Tabs, Tone,
} from "./ast.ts";
...
export function normalizeConstructs(tree: Root, source: string, diagnostics: Diagnostic[]): void {
  ...
  function normalizeDirective(node: Directive): Nodes {
    switch (node.name) {
      ...
      case "grid": return grid(node);
      case "columns": return columns(node);
      case "tabs": return tabs(node);
      case "steps": return steps(node);
      case "metrics": return metrics(node);
      case "figure": return figure(node);
      default: return node;
    }
  }
  ...
  function readAttributes<T extends Record<string, unknown>>(
    ...
    for (const [key, raw] of Object.entries(attributes.attrs)) {
      ...
      if (!rule) {
        report(ConstructCode.UNKNOWN_ATTRIBUTE, "warning", `"${key}" is not an attribute of ${owner}; it is ignored`, span);
        continue;
      }
      const value = rule.parse(raw);
      if (value === undefined) {
        report(ConstructCode.INVALID_ATTRIBUTE, "error", `${owner} ${key}="${raw}" is invalid; expected ${rule.expected}`, span);
        continue;
      }
  ...
  function card(node: Directive): Card {
    const a = readAttributes<{ tone: Tone; compact: boolean }>(node.attributes, {
      tone: oneOf(TONES, "neutral"),
      compact: boolean(false),
    }, fenceSpan(node), "card");
    return { type: "card", title: node.argument, tone: a.tone, compact: a.compact, ...base(node), children: node.children };
  }

  function grid(node: Directive): Grid | Directive {
    const a = readAttributes<{ cols: number; gap: Gap }>(node.attributes, {
      cols: integer(1, 4, 2),
      gap: oneOf(GAPS, "md"),
    }, fenceSpan(node), "grid");
  ...
  function metrics(node: Directive): Metrics | Directive {
    const a = readAttributes<{ direction: "normal" | "inverse" }>(node.attributes, {
      direction: oneOf(["normal", "inverse"] as const, "normal"),
    }, fenceSpan(node), "metrics");
    const [only] = node.children;
  ...
  function figure(node: Directive): Figure | Directive {
    const a = readAttributes<{ width: number | null }>(node.attributes, {
      width: {
        parse: (raw: string) => {
          const m = /^(\d+)%$/.exec(raw);
    ...
    }, fenceSpan(node), "figure");
    const [only] = node.children;
    const isImageParagraph = only?.type === "paragraph" && only.children.length === 1 && only.children[0].type === "image";
  ...
  function columns(node: Directive): Columns {
    ...
    const a = readAttributes<{ ratio: number[] | null; gap: Gap }>(node.attributes, {
      ratio: {
        parse: (raw: string) => {
    ...
    for (const child of node.children) {
      if (child.type === "separator") {
        ...
        current = { type: "column", id: sep.attributes.id, classes: sep.attributes.classes, children: [], position: sep.position };
        continue;
      }
    ...
    if (cols.length === 1) {
      report(ConstructCode.COLUMNS_SINGLE, "warning", "columns has a single column; add ::col to separate columns", span);
    }
    let ratio = a.ratio;
    if (ratio && ratio.length !== cols.length) {
      report(ConstructCode.COLUMNS_RATIO_MISMATCH, "error", `ratio has ${ratio.length} term(s) but there are ${cols.length} column(s)`, span);
      ratio = null;
    }
  ...
  function tabs(node: Directive): Tabs | Directive {
    ...
    if (headings.length === 0) {
      report(ConstructCode.TABS_NO_HEADINGS, "error", "tabs content must begin with a heading for each tab", wholeSpan(node));
      return node;
    }
    if (node.children[0].type !== "heading") {
      report(ConstructCode.TABS_CONTENT_BEFORE_HEADING, "error", "tabs content before the first heading belongs to no tab", wholeSpan(node.children[0]));
      return node;
    }
    const depth = headings[0].depth;
    const shallower = headings.find((h) => h.depth < depth);
    if (shallower) {
      report(ConstructCode.TABS_MIXED_LEVELS, "error", `tab headings must all be level ${depth}; found a level ${shallower.depth} heading`, wholeSpan(shallower));
      return node;
    }
    ...
    const a = readAttributes<{ active: number }>(node.attributes, {
  ...
  function normalizeCallout(node: Blockquote): Nodes {
    ...
    if (!first || first.type !== "paragraph") return node;
    ...
    if (!(CALLOUT_KINDS as readonly string[]).includes(kind)) {
      report(ConstructCode.CALLOUT_UNKNOWN_TYPE, "error",
        `"[!${match[1]}]" is not a callout type; expected ${CALLOUT_KINDS.map((k) => `[!${k}]`).join(", ")}`, wholeSpan(lead));
      return node;
    }
  ...
  function splitTitle(paragraph: Paragraph, markerLength: number): { title: PhrasingContent[] | null; body: Paragraph | null } {
