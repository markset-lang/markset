...
**A single series can be approximated in ASCII, and it is worth being exact about how thin the approximation is.** The distribution in that document, `#fig-dist`, is a horizontal bar chart written as an `ascii` fence: labels, bars of `#`, counts, an axis, and an arrow annotating one bucket. It is legible, and undrawn it is the same picture in text, which is the §10 argument for ASCII holding for charts as well as for diagrams.
...
**What the example could not chart is worth recording too.** `capacity-review.md` has four data figures and only two of them became charts. The others — the growth table and the cost table — each carry several measures on one row: petabytes beside tenant counts beside percentages, dollars per terabyte beside dollars per tenant. There is no honest single chart of those, and the alternative is the one thing a chart must never be, a second axis at a different scale. So the table stays a table. This is a real cost of "the table is the source": a table written to be read and a table written to be drawn are not always the same table, and the author has to choose. It is also the correct cost, because the failure mode it forces is "no chart" rather than "a misleading chart".
