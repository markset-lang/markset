...
Three of the remaining four are data tables that stayed tables because no honest single chart of them exists. `#fig-growth` puts petabytes beside tenant counts beside a percentage; `#fig-cost` puts dollars per terabyte beside dollars per tenant beside a share of the bill; `#fig-decision` has a column of prose in it. Charting any of those needs a second axis at a different scale, which is the one thing a chart must never be. The fourth, `#fig-dist`, was already a picture — the ASCII distribution plot — and §10 still draws it. This is a real cost of "the table is the source": a table written to be read and a table written to be drawn are not always the same table, and the author has to choose. It is also the correct cost, because the failure mode it forces is "no chart" rather than "a misleading chart".
