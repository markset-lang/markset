...
site/       static site generator (build.ts) and content; every page is Markset rendered by the packages above.
            ...
            document), and diagnostics.ts is the part testable outside a browser. An example with `toggles: true` in EXAMPLES gets every top-level construct
...
- `esbuild` (approved 2026-09-17) bundles `site/playground/app.ts` for the browser, and nothing else uses it.
  ...
  if that ever stops being true the test reports a skip with a reason instead of failing.
