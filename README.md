...
**v0, declared 2026-09-15.** Reference implementation `0.3.1`. The spec is implemented end to end (parser, an HTML renderer, a downgrade renderer that emits plain CommonMark, stylesheet, CLI), the conformance suite passes, and the §9 questions are decided. The eight packages are on npm under the `@markset-lang` scope. Six real documents were written against the candidate; the last four forced no change to §2 or §4, which is what v0 was waiting on. Changes within v0 are additive only — see the change policy in spec §0, `CHANGELOG.md`, and the status list in `CLAUDE.md`.
